...
This post breaks down why debug logs are losing their relevance — and what “modern observability” looks like when you stop staring at one line and start connecting all the dots.
...
Logs *used* to be enough. But then came microservices, clusters, autoscalers, load balancers, and all the other “must-have” cloud-native magic.

Yet somehow… we still reach for `logger.debug()` like it’s 2013.

If we’ve managed to leave *Harlem Shake*, *What Does the Fox Say*, and *Vine* behind — why can’t we do the same with debug logs?
...
Recently, we hit an out-of-memory issue in one of our components. Like most OpenTelemetry pipelines, we have several collectors, each responsible for its part of the flow. Looking at the logs, nothing seemed out of the ordinary.

So we added more logs. And then more.  
Eventually, we noticed something interesting — the retry mechanism was working overtime. Retries are part of the “happy path,” so we weren’t logging any errors. Only when we dove deep into debug level could we see how many spans were getting stuck in retry loops.

---

## It’s About the Destination, Not the Journey
After hopping from one collector to another, we eventually found the issue:  
The **destination** we were sending data to had high latency, causing repeated retries and backpressure throughout the pipeline.

It took hours — and several redeploys — to add enough logs, collect them, and piece together the pattern.
...
Each span shows a stop along the way — how long it took, and key details like queries, HTTP headers, or retry counts.

> One look at a trace, and you can see what a thousand log lines are trying to tell you.

*(Insert trace screenshot here)*
...
*(Insert trace screenshot showing retries and slow final destination)*
