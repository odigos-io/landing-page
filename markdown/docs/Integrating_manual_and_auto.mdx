...
This approach has few major advantages:
* It allows adding costum data that gets integrated into the trace seemlesly.
* It requires little code chages. As opposed to a fully manual setup in which the developer would have to define how exportion is done and configure different settings.
* From the Go application point of view, the API call such as `span.SetAttributes` and `span.End` are no-ops, since no SDK is not configured the defualt implemntation is a no-op. The actual collection is done using eBPF which leads to performance gains relative to manual instrumentation as described in ([this blog post](https://odigos.io/blog/ebpf-instrumentation-faster-than-manual)).
