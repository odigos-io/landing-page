...
We're developing [Odigos, an open-source project](https://github.com/keyval-dev/odigos) for effortless distributed tracing in all applications.

Distributed tracing is crucial for comprehensive monitoring, but we keep hearing about two major obstacles that prevent successful implementation: **extensive code changes and performance impact**.
...
As for the performance issues, we have some good news! We're excited to share the results of our performance tests, which show that **eBPF-based automatic instrumentation is over 20x faster than manually instrumenting code**.

As a result, you can now have the best of both worlds: effortless distributed tracing with minimal performance overhead.
