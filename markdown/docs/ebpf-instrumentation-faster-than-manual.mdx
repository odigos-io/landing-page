...
We have been working hard on solving this problem. Today, we're excited to share the results of our performance tests, which show that **eBPF-based automatic instrumentation is over 20x faster than manually instrumenting code.**
...
For maxium precision and isolation we are generating traces containing a single span. In a production environment we anticipate the performance difference to be even greater.
