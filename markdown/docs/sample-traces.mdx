...
2. For the Frontend service: 
    ...
    - Capture all requests to /products that exceed 1 ms.

### Here’s how to implement it:

First, we will create the Error Sampler action to capture 100% of error logs and retain only 5% of non-error logs.  
* Go to Actions > Add New Action > Error Sampler:  
![Creating Error Sampler Action](/images/blog/odigos-sampling/error-sampling.png)
<br>

Next, we’ll create the Latency Sampler action to capture specific requests in the Frontend service. We want to capture all requests to /products that exceed 1 ms and requests to /buy that exceed 1 second, while still keeping 10% of shorter traces.  
